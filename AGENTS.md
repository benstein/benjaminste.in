...
This is the canonical project instruction file for AI coding agents. Claude Code reads `CLAUDE.md`, which imports this file with `@AGENTS.md`; Codex reads this file directly.
...
Follow the active user's writing-style rules for any prose you generate for this site. In particular, avoid obvious AI tells: default em-dash rhythm, "it's not X, it's Y" as a reflex, tricolons for cadence, and generic abstraction where a concrete sentence would do.
...
- `llms.txt` — Jekyll renders it at build time via Liquid (`{% raw %}{% for post in site.posts %}{% endraw %}`). New posts show up automatically; do not edit by hand.
- `_site/` (gitignored; `bundle exec jekyll build` regenerates it).

## Images and icons

No emojis in rendered output. If an icon is needed, use inline SVG from Heroicons or Lucide.
